...
import { serializeGuideToMarkdown, type Guide } from "@/lib/guides";
...
const localeTagByGuideLanguage = {
  en: "en-US",
  es: "es-ES",
  pt: "pt-BR",
} as const;
...
export function createGuidePageMetadata({
  ...
  const { datePublished, dateModified } = getGuideContentDates(guide.language);
  const metadata = createPageMetadata({
    title: guide.title,
    description: guide.description,
    keywords: guide.keywords,
    path,
    locale: guide.language,
  });
  ...
  return {
    ...
    authors: [
      {
        name: founderName,
        url: absoluteUrl(getLocalizedPublicPath("/about", guide.language)),
      },
    ],
    openGraph: {
      ...metadata.openGraph,
      type: "article",
      url: path,
      locale: getOpenGraphLocale(guide.language),
      authors: [founderName],
...
export function GuideArticlePage({ guide }: { guide: Guide }) {
  ...
  const guidePath = getLocalizedPublicPath(`/guides/${guide.slug}`, guide.language);
  const guidesHubPath = getLocalizedPublicPath("/guides", guide.language);
  const aboutPath = getLocalizedPublicPath("/about", guide.language);
  const importPath = `${getLocalizedPublicPath("/", guide.language)}#upload-panel`;
  const { datePublished, dateModified } = getGuideContentDates(guide.language);
  const formattedPublishedDate = new Intl.DateTimeFormat(
    localeTagByGuideLanguage[guide.language],
    {
      dateStyle: "long",
    },
  ).format(new Date(datePublished));
  const formattedUpdatedDate = new Intl.DateTimeFormat(
    localeTagByGuideLanguage[guide.language],
    {
      dateStyle: "long",
    },
        ...
        itemListElement: [
          {
            ...
            position: 1,
            name: "Home",
            item: absoluteUrl(getLocalizedPublicPath("/", guide.language)),
          },
          {
            "@type": "ListItem",
            position: 2,
            name: "Guides",
            item: absoluteUrl(guidesHubPath),
          },
          {
            "@type": "ListItem",
            position: 3,
  ...
  const bylinePrefix = guide.language === "en" ? "By" : "Por";
      ...
      <div className="space-y-14 xl:space-y-16">
        <section className="grid gap-8 lg:grid-cols-[minmax(0,1.58fr)_minmax(24rem,0.82fr)] lg:items-end xl:gap-12">
          <div className="max-w-none">
            <p className="editorial-kicker text-(--accent-amber)">{guidesLabel}</p>
          ...
          <div className="max-w-none lg:pb-5 xl:pb-6">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-(--text-muted)">
              <span>{`${bylinePrefix} ${founderName}`}</span>
        ...
        >
          <header className="border-b border-(--border-soft) p-7 sm:p-9 xl:p-12">
            <p className="guide-article-intro text-(--text-muted)">{guide.intro}</p>
            ...
            <section className="mt-9 rounded-[1.6rem] border border-(--border-soft) bg-(--surface-soft) p-6 sm:p-7">
              <div className="grid gap-6 xl:grid-cols-[minmax(18rem,0.78fr)_minmax(0,1.22fr)] xl:gap-8">
                <div>
                  <p className="editorial-kicker text-(--accent-sky)">{audienceLabel}</p>
                  <p className="mt-4 text-[1.02rem] leading-8 text-(--text-muted)">
                    {guide.audience}
                  </p>
                </div>
                <div>
                  <p className="editorial-kicker text-(--accent-amber)">{takeawaysLabel}</p>
          ...
          <div className="p-7 sm:p-9 xl:p-12">
            ...
            <section className="border-t border-(--border-soft) pt-12 xl:pt-14">
              ...
              <div className="mt-7 grid gap-4 lg:grid-cols-3">
                ...
                >
                  <p className="text-sm font-semibold text-(--text-strong)">{hubLabel}</p>
                ...
                <Link
                  ...
                  className="rounded-[1.35rem] border border-(--border-soft) bg-(--surface-soft) px-5 py-5 transition hover:border-(--border-strong) hover:bg-(--surface-chip)"
                >
                  <p className="text-sm font-semibold text-(--text-strong)">{aboutLabel}</p>
                  <p className="mt-2 text-sm leading-7 text-(--text-muted)">
                    {guide.language === "es"
                ...
                <Link
                  ...
                  className="rounded-[1.35rem] border border-(--border-soft) bg-(--surface-soft) px-5 py-5 transition hover:border-(--border-strong) hover:bg-(--surface-chip)"
                >
                  <p className="text-sm font-semibold text-(--text-strong)">{importLabel}</p>
                  <p className="mt-2 text-sm leading-7 text-(--text-muted)">
                    {guide.language === "es"
