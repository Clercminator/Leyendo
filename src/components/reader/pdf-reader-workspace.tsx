...
export function PdfReaderWorkspace({
    ...
    async function openPdfDocument() {
      ...
      try {
        const [asset, storedViewerState, nextPdfViewerModule] =
          await Promise.all([
            getDocumentAsset(document.id),
            getStoredPdfViewerState(document.id),
            loadPdfJsViewer(),
          ]);
  ...
  }, [document.id, onPageChange, pageLabels, pdfDocument, pdfViewerModule]);
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      void savePdfViewerState(document.id, viewerState);
    }, 180);

    return () => {
      window.clearTimeout(timeoutId);
    };
  }, [document.id, pdfDocument, viewerState]);
